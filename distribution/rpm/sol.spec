...
cp -a %{_builddir}/bin/sol %{buildroot}%{_bindir}
cp -a %{_builddir}/config/sol.yaml %{buildroot}%{_sysconfdir}/%{_name}/sol.yaml
cp -a %{_builddir}/config/examples/. %{buildroot}%{_sysconfdir}/%{_name}/examples
cp -a %{_builddir}/systemd/sol.service %{buildroot}%{_unitdir}/sol.service
cp -a %{_builddir}/systemd/sol.default %{buildroot}%{_sysconfdir}/default/sol
...
getent passwd %{_username} > /dev/null || \
  useradd --shell /sbin/nologin --system --home-dir %{_sharedstatedir}/%{_name} --user-group \
    --comment "Sol observability data router" %{_username}
...
%defattr(-,root,root,-)
%{_bindir}/*
%{_unitdir}/sol.service
%config(noreplace) %{_sysconfdir}/%{_name}/sol.yaml
%config(noreplace) %{_sysconfdir}/default/sol
