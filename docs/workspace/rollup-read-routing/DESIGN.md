...
The choke point routes to a tier **only** when the query's value computation is preserved by last-sample-per-bucket downsampling. The allowlist is scoped to **what the querier actually implements today** (the range-function dispatch at `prometheus.rs:333-353` supports exactly `rate`/`increase`/`irate`/`{max,min,avg,sum,count}_over_time`; every other range function errors). Rollup-**safe**: `rate`, `increase`, and `histogram_quantile` (cumulative bucket counts). Rollup-**unsafe** (force raw): `irate` (last-two-samples slope), `max_over_time`, `min_over_time`, `avg_over_time`, `sum_over_time`, `count_over_time`, and any operator needing intra-bucket detail. A **bare selector** (no `Call`) has no safe operator → raw. Unknown/unimplemented/unclassified operators default to **raw** (conservative). `delta`/`resets`/`last_over_time` would be safe and `quantile_over_time` unsafe — but none are implemented yet, so they stay in the default-raw bucket until added (see the [operator-safety ADR](./adrs/operator-safety-allowlist.md)).
