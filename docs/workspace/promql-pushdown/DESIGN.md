...
- **In-memory Rust (the cost):** `without(…)` grouping, **nested aggregates** (`count(count(…) by (cpu))`), `topk`/`bottomk`, `scalar()`, `clamp_min/max`, all binary/unary ops, and `histogram_quantile`. These run via `aggregate_instant_vector`/`aggregate_range_series` (the [promql-aggregate-evaluation ADR](../parquet-backend/adrs/querier/promql-aggregate-evaluation.md)).
...
Not yet in production → **clean cutover, no retro-compat or backfill** (regenerate the Parquet store, exactly as the [prom-name-column](../prom-name-column/TASKS.md) migration did).
...
### <a id="fr4"></a>FR4 — Materialized hot-label columns (endgame)
The codec writes a **bounded, configured set** of "hot" labels as real Parquet columns alongside `prom_name` (same pattern: schema field, populated at write time, part of the sort key where useful). The read side groups/filters on these columns when present (prunable: row-group min/max + bloom filters), falling back to the `attributes` JSON for non-materialized labels. Clean cutover — the store is regenerated, old files are not read.
...
- **In-memory hot tier (ingester-style).** Already an explicit non-goal of [parquet-backend](../parquet-backend/DESIGN.md#nfr6); freshness stays capped at the flush interval. This migration is about *how* flushed data is processed, not adding a RAM tier.
- **Intraday rollups for the active day.** A real lever for same-day long ranges, but it belongs to the rollup lifecycle ([long-range-metrics-strategy](../parquet-backend/adrs/compactor/long-range-metrics-strategy.md)), not the PromQL engine. Excluded here to keep scope coherent; can be a separate workspace.
...
- **Materializing *all* attribute keys as columns.** Unbounded distinct keys → unbounded sparse columns → schema explosion. *Cap:* FR4 materializes a **bounded configured allowlist** of hot labels only ([materialized-label-columns ADR](./adrs/materialized-label-columns.md)); everything else stays in `attributes` JSON.
...
Write-side (FR4) extends the metric Parquet schema (per [materialized-label-columns ADR](./adrs/materialized-label-columns.md)) with a bounded set of hot-label columns next to `prom_name`.

Decisions:
- [Aggregation pushdown — relational core in DataFusion](./adrs/aggregation-pushdown.md) — *supersedes* [promql-aggregate-evaluation](../parquet-backend/adrs/querier/promql-aggregate-evaluation.md)
- [Group-key canonical format](./adrs/group-key-format.md)
- [Relational / non-relational boundary](./adrs/relational-nonrelational-boundary.md)
- [Materialized hot-label columns (endgame, clean cutover)](./adrs/materialized-label-columns.md)
