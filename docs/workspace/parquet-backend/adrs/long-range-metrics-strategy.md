...
Metrics are queried over **90 days – 2 years** ([NFR7](../DESIGN.md#nfr7)), and Grafana re-issues the same range every ~15s. Two failure modes:
1. A 2y range scanned at raw resolution is infeasible (cost, memory) — even with pruning.
...
Traces (<7d) and logs (<30d) do not have this problem — short windows. So the strategy is metrics-specific.
...
- This applies to metrics only. The catalog still registers traces/logs as plain day-partitioned tables without splitting/rollups.
