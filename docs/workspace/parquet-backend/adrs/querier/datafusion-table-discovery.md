...
- Each table's Arrow schema is **declared explicitly in code** (not inferred) from the column lists in [parquet-multisignal](../../../../parquet-multisignal/designs/20260527_parquet-multisignal.md) — the binding codec↔query contract; schema mismatch is a hard error.
...
- Attribute filters use `json_*` extraction on the JSON-string `attributes`/`resource_attributes`/`scope_attributes` columns ([ADR 0038](../../../../parquet-multisignal/adrs/20260527_attributes-serialization-strategy.md)); these do not push down into Parquet — accepted per [rabbit hole 4](../../DESIGN.md#rabbit-holes).
