...
> `62c0334b4` Task 5). `querier::` 136 passed; `sol-core` + `codecs` parquet tests
...
> is a live-stack check on the rebuilt `sol-querier` image (deferred; needs docker).
> Phase 6 (docs integration) awaits go-ahead.

## Analysis

Build: `cargo build --features querier-backend --lib` — verified compiles green
Test (query): `cargo test --features querier-backend --lib querier::` — green baseline (138 passed)
...
Lint: `cargo clippy --features querier-backend --lib -- -D warnings` + `cargo clippy -p codecs -- -D warnings`
...
| Test | Reason | Action |
|---|---|---|
| (none in `querier::` / codec) | baseline green | — |
...
| Concern | Location |
|---|---|
| Normalizer (move source) | `src/querier/udf.rs:88` `unit_suffix`, `:119` `prom_metric_name` |
...
| UDF wrapper (keep, delegate) | `src/querier/udf.rs:150` `prom_metric_name_udf` |
...
| Catalog read schema | `src/querier/catalog.rs:142` `metric_union_schema` (must mirror codec) |
| UDF registration | `src/querier/catalog.rs:365` `register_udf(prom_metric_name_udf())` (keep — FR6 fallback) |
| Read filter sites | `src/querier/prometheus.rs`: `prom_name_expr` :39, `name_pred_expr` :63, and callers :104, :147/155, :303/311, :667–668 (`__name__` label_values), :979, :1138, :1503 (histogram/bucket scans) |
| Compaction sort | `src/querier/compaction.rs` (sort-merge by `service_name, time_col`) |
| Rollup round-trip | `src/querier/rollup.rs` (`rollup_batches` — column must survive) |
...
**Constraints**:
- [ADR: normalizer-canonical-location](./adrs/normalizer-canonical-location.md) — move the **pure** fns to `lib/sol-core`. The `prom_metric_name_udf` wrapper in `src/querier/udf.rs` stays for now and delegates to the moved fn (it is **deleted in Task 4** with the read switch, so Session 1 stays green).
...
**Verify**: `cargo test -p sol-core prom_metric_name && cargo test --features querier-backend --lib querier::udf::`
**Acceptance**:
...
**Verify**: `cargo test --features querier-backend --lib querier::catalog::`
**Acceptance**:
...
- **Delete** `prom_metric_name_udf` from `src/querier/udf.rs`, its registration at `src/querier/catalog.rs:365`, and the `udf.rs` wrapper test. No read site may reference the UDF afterward.
- Update the `querier::` test fixtures (in-memory `MemTable`s for `handle_*`) to include a `prom_name` column so the column filter resolves.
...
**Verify**: `cargo test --features querier-backend --lib querier::compaction:: querier::rollup::`
**Acceptance**:
...
- [NFR2](./DESIGN.md#nfr2): full `querier::` suite green at parity.
...
**Checkpoint**: `cargo test -p sol-core && cargo test -p codecs && cargo test --features querier-backend --lib querier:: && cargo clippy --features querier-backend --lib -- -D warnings`
(green = new column written + schema mirrored; read still uses the UDF, so `querier::` stays green at parity)
**Commit point**: yes — after checkpoint passes

### Session 2 — read switch + migration + verify (~2.5H)
Tasks: 4, 5, 6
**Skills**: `rust-software-engineer`
**Checkpoint**: `cargo test --features querier-backend --lib querier:: && cargo clippy --features querier-backend --lib -- -D warnings` (+ EXPLAIN pruning check)
**Commit point**: yes
