...
3. **Bounded caches** — a Parquet metadata/data cache and the query-result cache ([caching ADR](../querier/query-caching-strategy.md)) share a total memory budget capped by NFR5 (default ≤256 MB). This is the memory⇄latency knob: raise the budget for lower latency, lower it to protect ingestion.
