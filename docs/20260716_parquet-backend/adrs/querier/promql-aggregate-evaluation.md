...
Addresses: [FR1](../../designs/parquet-backend.md#fr1) (Prometheus-compatible API), [FR4](../../designs/parquet-backend.md#fr4) (DataFusion engine), [NFR5](../../designs/parquet-backend.md#nfr5) (memory budget), [NFR6](../../designs/parquet-backend.md#nfr6) (latency). Refines the [expr-lowering design](../../../../20260608_expr-lowering/designs/20260608_expr-lowering.md) (PromQL → `Expr`/`DataFrame`, no `format!` SQL).
...
- **[NFR9](../../designs/parquet-backend.md#nfr9) guardrails** (`max_bytes_scanned`, per-signal `max_range`, `max_concurrent_queries`) cap the leaf scan and concurrency → an indirect bound on what an aggregate can materialise (a query that would scan too much is rejected 422 before the reduce).
