...
- **Stack dependency upgrade (tonic 0.12 → 0.13+, hyper 0.14 → 1.x)**: valuable but separate scope. The tuning parameters use the same API across versions — optimize first, upgrade later. ~~Recommended as the next step after this work lands.~~ **Update**: [tonic-stack-upgrade research](../../tonic-stack-upgrade/designs/20260518_tonic-stack-upgrade.md) found no documented throughput improvement from the upgrade — hyper 1.x is an API redesign, not a performance release.
