...
Avro remains available as a per-event serializer (`SerializerConfig::Avro`) for sinks that need row-based encoding (e.g., Kafka). It is not suitable as the storage format for the observability backend storage path.
