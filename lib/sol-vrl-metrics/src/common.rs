...
use sol_core::{event::OtelMetric, metrics::Controller};
...
pub struct MetricsStorage {
    ...
    #[doc(hidden)]
    pub cache: Arc<ArcSwap<Vec<OtelMetric>>>,
}
...
fn tag_matches(metric: &OtelMetric, (tag_key, tag_value): (&String, &String)) -> bool {
...
pub(crate) fn metric_into_vrl(value: &OtelMetric) -> Value {
    value!({
        name: { value.name() },
        tags: {
            BTreeMap::from_iter(
        ...
        "type": {
            match value.view() {
                sol_core::event::MetricView::Sum { .. } => "counter",
                sol_core::event::MetricView::Gauge { .. } => "gauge",
                sol_core::event::MetricView::Set { .. } => "set",
                sol_core::event::MetricView::Histogram { .. } => "aggregated histogram",
                sol_core::event::MetricView::Summary { .. } => "aggregated summary",
                sol_core::event::MetricView::ExponentialHistogram { .. } => "exponential histogram",
            }
        },
        kind: {
            match value.kind() {
                sol_core::event::MetricKind::Incremental => "incremental",
                sol_core::event::MetricKind::Absolute => "absolute",
            }
        },
        value: {
            match value.view() {
                sol_core::event::MetricView::Sum { value }
                | sol_core::event::MetricView::Gauge { value } => NotNan::new(value).ok(),
                _ => None,
            }
        }
    })
...
mod tests {
    use sol_core::{
        compile_vrl,
        event::{Event, OtelAttributes, OtelLog, VrlTarget},
    };
    use vrl::{
        compiler::{
    ...
    fn compile_and_run(storage: MetricsStorage, vrl_source: &str) -> Result<Value, Terminate> {
        ...
        let mut target = VrlTarget::new(Event::Log(OtelLog::default()), program.info(), false);
    ...
    fn assert_metric_matches(
        ...
        if let Some(tags) = tags {
            let otel_tags = metric.get("tags").unwrap().as_object().unwrap();
