...
use super::{
    ...
    format::{
        ...
        LogfmtSerializerConfig, ProtobufSerializer, ProtobufSerializerConfig, RawMessageSerializer,
...
pub enum BatchSerializerConfig {
    ...
    /// Encodes events as [Apache Parquet][parquet] files with column-level compression.
    ///
    /// Each batch produces a complete Parquet file (header + row groups + footer).
    ///
...
impl BatchSerializerConfig {
    /// Build a `BatchSerializer` from this configuration.
    pub fn build(
        &self,
...
impl Serializer {
    ...
    pub fn supports_json(&self) -> bool {
        match self {
            Serializer::Json(_) | Serializer::Gelf(_) => true,
            Serializer::Avro(_)
            | Serializer::Cef(_)
            | Serializer::Csv(_)
            | Serializer::Logfmt(_)
