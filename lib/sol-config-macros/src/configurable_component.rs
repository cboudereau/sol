    ...
    fn get_component_desc_registration(
        ...
        self.component_name.as_ref().map(|component_name| {
            ...
            let desc_ty: syn::Type = match self.component_type {
                ComponentType::Api => {
                    parse_quote! { ::sol_config::component::ApiDescription }
                }
                ComponentType::EnrichmentTable => {
                    parse_quote! { ::sol_config::component::EnrichmentTableDescription }
                }
                ComponentType::GlobalOption => {
                    parse_quote! { ::sol_config::component::GlobalOptionDescription }
                }
                ComponentType::Provider => {
                    parse_quote! { ::sol_config::component::ProviderDescription }
                }
                ComponentType::Secrets => {
                    parse_quote! { ::sol_config::component::SecretsDescription }
                }
                ComponentType::Sink => parse_quote! { ::sol_config::component::SinkDescription },
                ComponentType::Source => {
                    parse_quote! { ::sol_config::component::SourceDescription }
                }
                ComponentType::Transform => {
                    parse_quote! { ::sol_config::component::TransformDescription }
                }
            };
    ...
    fn get_component_name_registration(&self) -> proc_macro2::TokenStream {
        ...
        match self.component_name.as_ref() {
            None => quote_spanned! {self.span=>
                #[derive(::sol_config::NamedComponent)]
                #[#helper_attr]
            },
