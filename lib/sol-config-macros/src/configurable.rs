...
pub fn derive_configurable_impl(input: TokenStream) -> TokenStream {
    ...
    if !generic_field_types.is_empty() {
        ...
        for typ in generic_field_types {
            let ty = &typ.ident;
            let predicate = parse_quote! { #ty: ::sol_config::Configurable + ::serde::Serialize + ::sol_config::ToValue };
    ...
    let configurable_impl = quote! {
        const _: () = {
            ...
            impl #impl_generics ::sol_config::Configurable for #name #ty_generics #where_clause {
            ...
            impl #impl_generics ::sol_config::ToValue for #name #ty_generics #where_clause {
                #to_value_fn
            }
        };
    };
...
fn build_metadata_fn(container: &Container<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn metadata() -> ::sol_config::Metadata {
            #container_metadata
            #meta_ident
        }
    }
...
fn build_to_value_fn(_container: &Container<'_>) -> proc_macro2::TokenStream {
    quote! {
        fn to_value(&self) -> ::sol_config::serde_json::Value {
            ::sol_config::serde_json::to_value(self)
...
fn build_virtual_newtype_schema_fn(virtual_ty: Type) -> proc_macro2::TokenStream {
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<::sol_config::schema::SchemaGenerator>) -> std::result::Result<::sol_config::schema::SchemaObject, ::sol_config::GenerateError> {
            ::sol_config::schema::get_or_generate_schema(
                &<#virtual_ty as ::sol_config::Configurable>::as_configurable_ref(),
                schema_gen,
                None,
            )
        }
...
fn build_enum_generate_schema_fn(
    ...
    let generate_block = quote! {
        if ::sol_config::schema::has_ambiguous_discriminants(&discriminant_map) {
            Ok(::sol_config::schema::generate_any_of_schema(&subschemas))
        } else {
            Ok(::sol_config::schema::generate_one_of_schema(&subschemas))
        }
    };
...
fn generate_struct_field(field: &Field<'_>) -> proc_macro2::TokenStream {
    ...
    let spanned_generate_schema = quote_spanned! {field.span()=>
        ::sol_config::schema::get_or_generate_schema(
            &<#field_schema_ty as ::sol_config::Configurable>::as_configurable_ref(),
            schema_gen,
...
fn generate_named_struct_field(
    ...
    let integrate_field = if field.flatten() {
        ...
        let spanned_is_optional = quote_spanned! {field.span()=>
            <#field_schema_ty as ::sol_config::Configurable>::is_optional()
        };
...
fn build_named_struct_generate_schema_fn(
    ...
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<::sol_config::schema::SchemaGenerator>) -> std::result::Result<::sol_config::schema::SchemaObject, ::sol_config::GenerateError> {
            let mut properties = ::sol_config::indexmap::IndexMap::new();
            let mut required = ::std::collections::BTreeSet::new();
            let mut flattened_subschemas = ::std::vec::Vec::new();

            let metadata = <Self as ::sol_config::Configurable>::metadata();
            #(#mapped_fields)*

            let had_unflatted_properties = !properties.is_empty();

            let additional_properties = None;
            let mut schema = ::sol_config::schema::generate_struct_schema(
                properties,
                required,
            ...
            if !flattened_subschemas.is_empty() {
                ...
                ::sol_config::schema::convert_to_flattened_schema(&mut schema, flattened_subschemas);
            }
...
fn build_tuple_struct_generate_schema_fn(fields: &[Field<'_>]) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<::sol_config::schema::SchemaGenerator>) -> std::result::Result<::sol_config::schema::SchemaObject, ::sol_config::GenerateError> {
            ...
            Ok(::sol_config::schema::generate_tuple_schema(&subschemas))
        }
    }
}
...
fn generate_container_metadata(
    ...
    quote! {
        let mut #meta_ident = ::sol_config::Metadata::default();
        #maybe_title
        #maybe_description
        #maybe_default_value
        #maybe_deprecated
...
fn generate_named_enum_field(field: &Field<'_>) -> proc_macro2::TokenStream {
    ...
    let spanned_is_optional = quote_spanned! {field.span()=>
        <#field_ty as ::sol_config::Configurable>::is_optional()
    };
...
fn generate_enum_variant_tag_schema(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        {
            let mut tag_subschema = ::sol_config::schema::generate_const_string_schema(#variant_name.to_string());
    ...
    let variant_schema = match variant.tagging() {
        ...
        Tagging::Internal { tag } => match variant.style() {
            ...
            Style::Newtype => {
                ...
                quote! {
                    let tag_schema = ::sol_config::schema::generate_internal_tagged_variant_schema(#tag.to_string(), #tag_schema);
                    let mut flattened_subschemas = ::std::vec::Vec::new();
                    flattened_subschemas.push(tag_schema);

                    let mut newtype_schema = #newtype_schema;
                    ::sol_config::schema::convert_to_flattened_schema(&mut newtype_schema, flattened_subschemas);
        ...
        Tagging::Adjacent { tag, content } => {
            ...
            quote! {
                let mut wrapper_properties = ::sol_config::indexmap::IndexMap::new();
                ...
                ::sol_config::schema::generate_struct_schema(
                    wrapper_properties,
                    wrapper_required,
                    None
                )
        ...
        Tagging::None => {
            ...
            match variant.style() {
                ...
                Style::Unit => quote! { ::sol_config::schema::generate_null_schema() },
            }
        }
    };
...
fn generate_enum_variant_apply_metadata(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        #variant_metadata
        ::sol_config::schema::apply_base_metadata(&mut subschema, #variant_metadata_ref);
    }
}
...
fn generate_enum_variant_tag_apply_metadata(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        ::sol_config::schema::apply_base_metadata(&mut tag_subschema, #variant_tag_metadata_ref);
    }
}
