...
fn http_encode_event_text() {
    let event = Event::Log(OtelLog::from_str_legacy("hello world"));
...
async fn http_passes_template_headers() {
    ...
    "#,
        || {
            let mut event = Event::Log(OtelLog::from_str_legacy("test message"));
...
async fn http_template_headers_missing_fields() {
    ...
    "#,
        || {
            let mut event = Event::Log(OtelLog::from_str_legacy("good event"));
...
async fn missing_field_in_uri_template() {
    ...
    let (batch, mut receiver) = BatchNotifier::new_with_receiver();
    let mut event = Event::Log(OtelLog::default());
