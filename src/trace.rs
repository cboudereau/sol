...
static BUFFER: Mutex<Option<Vec<OtelLog>>> = Mutex::new(Some(Vec::new()));
...
static SUBSCRIBERS: Mutex<Option<Vec<oneshot::Sender<Vec<OtelLog>>>>> =
...
static SENDER: OnceLock<Sender<OtelLog>> = OnceLock::new();
...
#[cfg(test)]
pub fn reset_early_buffer() -> Option<Vec<OtelLog>> {
    get_early_buffer().replace(Vec::new())
}

/// Gets a  mutable reference to the early buffer.
fn get_early_buffer() -> MutexGuard<'static, Option<Vec<OtelLog>>> {
    BUFFER
        .lock()
...
fn try_buffer_event(log: &OtelLog) -> bool {
...
fn try_broadcast_event(log: OtelLog) {
...
fn consume_early_buffer() -> Vec<OtelLog> {
    get_early_buffer()
        .take()
...
fn get_trace_sender() -> &'static broadcast::Sender<OtelLog> {
...
fn maybe_get_trace_sender() -> Option<&'static broadcast::Sender<OtelLog>> {
    SENDER.get()
}
...
fn get_trace_receiver() -> broadcast::Receiver<OtelLog> {
    get_trace_sender().subscribe()
}

/// Gets a mutable reference to the list of waiting subscribers, if it exists.
fn get_trace_subscriber_list() -> MutexGuard<'static, Option<Vec<oneshot::Sender<Vec<OtelLog>>>>> {
...
fn try_register_for_early_events() -> Option<oneshot::Receiver<Vec<OtelLog>>> {
    if SHOULD_BUFFER.load(Ordering::Acquire) {
...
/// specifically `OtelLog`, such that they can be shuttled around and treated as normal events.  Currently only powers
/// the `internal_logs` source, but could be used for other purposes if need be.
pub struct TraceSubscription {
    buffered_events_rx: Option<oneshot::Receiver<Vec<OtelLog>>>,
    trace_rx: Receiver<OtelLog>,
}
...
impl TraceSubscription {
    ...
    pub async fn buffered_events(&mut self) -> Option<Vec<OtelLog>> {
    ...
    pub fn into_stream(self) -> impl Stream<Item = OtelLog> + Unpin {
    ...
    S: Subscriber + 'static + for<'lookup> LookupSpan<'lookup>,
{
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        if should_process_tracing_event() {
            let mut log = OtelLog::from_tracing_event(event);
